...
$stmt = $pdo->prepare("SELECT d.*, r.name AS room_name FROM devices d JOIN rooms r ON d.room_id=r.id WHERE d.id=? AND d.type='ПК'");
$stmt->execute([$device_id]);
$device = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$device) die("Компьютер не найден.");

// Железо
$hw = $pdo->prepare("SELECT * FROM computer_hardware WHERE device_id=?");
...
// Открытые проблемы
$issuesByComp = [];
$stmt2 = $pdo->prepare("SELECT component FROM computer_issues WHERE device_id=? AND resolved_at IS NULL");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update'])) {
        ...
        if ($name === '') { $error = 'Название обязательно.'; }
            ...
            if (!empty($changes)) {
                ...
                $pdo->prepare("INSERT INTO computer_history (device_id,action,field_name,old_value,new_value,note) VALUES (?,?,?,?,?,?)")
            ...
            $pdo->prepare("INSERT INTO computer_hardware
    ...
    if (isset($_POST['duplicate'])) {
        $pdo->prepare("INSERT INTO devices (room_id,name,type,ip,mac,inventory_number,status,comment,icon) VALUES (?,?,?,?,?,?,?,?,?)")
            ->execute([$device['room_id'],$device['name'].' (копия)','ПК',$device['ip'],$device['mac'],$device['inventory_number'],$device['status'],$device['comment'],$device['icon']]);
        ...
        if ($hw) {
            ...
            $pdo->prepare("INSERT INTO computer_hardware (device_id,$keys) VALUES (?,$vals)")
