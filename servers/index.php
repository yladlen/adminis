...
$sql  = "SELECT d.*, r.name AS room_name,
         sh.cpu, sh.ram_gb, sh.hdd_gb, sh.os, sh.form_factor,
         sh.manufacturer, sh.model, sh.serial_number, sh.year_manufactured,
         sh.commissioned_at, sh.warranty_until, sh.floor,
         (SELECT COUNT(*) FROM server_issues ci WHERE ci.device_id=d.id AND ci.resolved_at IS NULL) AS open_issues
         FROM devices d
         LEFT JOIN rooms r ON r.id = d.room_id
         LEFT JOIN server_hardware sh ON sh.device_id = d.id";
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    fputcsv($out, [
        ...
        'Процессор', 'ОЗУ (ГБ)', 'Диск', 'ОС/Гипервизор', 'Форм-фактор',
...
<div class="content-wrapper">
    <div class="content-container">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="mb-0" style="text-align:left">Серверы</h1>
            <div class="d-flex gap-2">
                <a href="?<?= http_build_query(array_filter(array_merge($filters, ['export'=>'csv']))) ?>" class="btn btn-outline-success">⬇ CSV</a>
                <a href="add.php" class="btn btn-outline-success">➕ Добавить сервер</a>
            </div>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-3" style="font-size:13px;color:#6b7499">
            <span id="count-label">Всего: <?= count($items) ?> серверов</span>
        ...
        <?php if (empty($items)): ?>
            <p class="p-center">Сервер не найдены.</p>
        <?php else: ?>
            <div class="table-responsive">
                <table id="main-table">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr onclick="location.href='edit.php?id=<?= $item['id'] ?>&from=servers'" style="cursor:pointer">
