...
$stmt = $pdo->prepare("SELECT d.*, r.name AS room_name FROM devices d JOIN rooms r ON d.room_id=r.id WHERE d.id=? AND d.type='Ноутбук'");
$stmt->execute([$device_id]);
$device = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$device) die("Ноутбук не найден.");

// Железо
$hw = $pdo->prepare("SELECT * FROM notebook_hardware WHERE device_id=?");
...
// Открытые проблемы
$issuesByComp = [];
$stmt2 = $pdo->prepare("SELECT component FROM notebook_issues WHERE device_id=? AND resolved_at IS NULL");
$stmt2->execute([$device_id]);
foreach ($stmt2->fetchAll() as $row) if ($row['component']) $issuesByComp[$row['component']] = true;

$openIssues = $pdo->prepare("SELECT * FROM notebook_issues WHERE device_id=? AND resolved_at IS NULL ORDER BY reported_at DESC");
$openIssues->execute([$device_id]);
$openIssues = $openIssues->fetchAll(PDO::FETCH_ASSOC);

// История
$history = $pdo->prepare("SELECT * FROM notebook_history WHERE device_id=? ORDER BY created_at DESC LIMIT 50");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_issue'])) {
        ...
        if ($desc !== '') {
            $pdo->prepare("INSERT INTO notebook_issues (device_id,component,description) VALUES (?,?,?)")
                ->execute([$device_id, $comp ?: null, $desc]);
            $pdo->prepare("INSERT INTO notebook_history (device_id,action,field_name,new_value) VALUES (?,?,?,?)")
    ...
    if (isset($_POST['resolve_issue'])) {
        ...
        $pdo->prepare("UPDATE notebook_issues SET resolved_at=NOW(),resolution=? WHERE id=? AND device_id=?")
            ->execute([$resolution ?: null, $issue_id, $device_id]);
        $iss = $pdo->prepare("SELECT component,description FROM notebook_issues WHERE id=?");
        $iss->execute([$issue_id]);
        $issRow = $iss->fetch(PDO::FETCH_ASSOC);
        $pdo->prepare("INSERT INTO notebook_history (device_id,action,field_name,old_value,new_value) VALUES (?,?,?,?,?)")
    ...
    if (isset($_POST['update'])) {
        ...
        if ($name === '') { $error = 'Название обязательно.'; }
            ...
            $hwFields = [
                ...
                'hw_gpu'          => ['key'=>'gpu',              'label'=>'Видеокарта'],
                'hw_os'           => ['key'=>'os',               'label'=>'ОС'],
            ...
            if (!empty($changes)) {
                ...
                $pdo->prepare("INSERT INTO notebook_history (device_id,action,field_name,old_value,new_value,note) VALUES (?,?,?,?,?,?)")
            ...
            $pdo->prepare("INSERT INTO notebook_hardware
                ...
                cpu=VALUES(cpu),ram_gb=VALUES(ram_gb),hdd_gb=VALUES(hdd_gb),gpu=VALUES(gpu),os=VALUES(os),
    ...
    if (isset($_POST['duplicate'])) {
        $pdo->prepare("INSERT INTO devices (room_id,name,type,ip,mac,inventory_number,status,comment,icon) VALUES (?,?,?,?,?,?,?,?,?)")
            ->execute([$device['room_id'],$device['name'].' (копия)','Ноутбук',$device['ip'],$device['mac'],$device['inventory_number'],$device['status'],$device['comment'],$device['icon']]);
        ...
        if ($hw) {
            ...
            $pdo->prepare("INSERT INTO notebook_hardware (device_id,$keys) VALUES (?,$vals)")
...
<div class="d-flex justify-content-between align-items-center mb-4" style="gap:16px;flex-wrap:wrap">
    ...
    <div style="display:flex;align-items:center;gap:6px;flex-wrap:nowrap">
        ...
        <button type="submit" name="delete" class="btn btn-outline-danger" onclick="return confirm('Удалить ноутбук?')">🗑️ Удалить</button>
        <button type="submit" name="duplicate" class="btn btn-outline-success" onclick="return confirm('Дублировать ноутбук?')">📋 Дублировать</button>
...
<div class="form-section">
    ...
    <div class="row">
        <div class="col-md-6" style="margin-bottom:14px">
            ...
            <input type="text" name="hw_cpu" class="form-control" placeholder="Intel Core i5-10400" value="<?= htmlspecialchars($hw['cpu'] ?? '') ?>">
        </div>
        <div class="col-md-3" style="margin-bottom:14px">
            <label class="form-label">ОЗУ (ГБ)</label>
            <input type="text" name="hw_ram" class="form-control" placeholder="8" value="<?= htmlspecialchars($hw['ram_gb'] ?? '') ?>">
        </div>
        <div class="col-md-3" style="margin-bottom:14px">
            <label class="form-label">Диск</label>
            <input type="text" name="hw_hdd" class="form-control" placeholder="512 SSD" value="<?= htmlspecialchars($hw['hdd_gb'] ?? '') ?>">
        </div>
        <div class="col-md-6" style="margin-bottom:14px">
            <label class="form-label">Видеокарта</label>
            <input type="text" name="hw_gpu" class="form-control" placeholder="Встроенная / GTX 1050" value="<?= htmlspecialchars($hw['gpu'] ?? '') ?>">
        </div>
        <div class="col-md-6">
            <label class="form-label">Операционная система</label>
            <input type="text" name="hw_os" class="form-control" placeholder="Windows 10 Pro" value="<?= htmlspecialchars($hw['os'] ?? '') ?>">
        </div>
    </div>
</div>
...
<div class="side-panel">
    ...
    <div class="panel-box">
        ...
        <div>
            ...
            <?php if (!empty($hw['gpu'])): ?>
                <div class="passport-row">
                    <span class="passport-label">GPU</span>
                    <span class="passport-value" style="font-size:12px"><?= htmlspecialchars($hw['gpu']) ?></span>
                </div>
            <?php endif; ?>
            <?php if (!empty($hw['os'])): ?>
                <div class="passport-row">
