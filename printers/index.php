...
$sql  = "SELECT d.*, r.name AS room_name,
         pp.manufacturer, pp.model, pp.serial_number, pp.year_manufactured,
         pp.commissioned_at, pp.warranty_until, pp.connection_type, pp.network_support, pp.floor,
         (SELECT COUNT(*) FROM printer_issues ci WHERE ci.device_id=d.id AND ci.resolved_at IS NULL) AS open_issues
         FROM devices d
         LEFT JOIN rooms r ON r.id = d.room_id
         LEFT JOIN printer_passport pp ON pp.device_id = d.id";
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="printers_' . date('Y-m-d') . '.csv"');
    $out = fopen('php://output', 'w');
    fprintf($out, chr(0xEF).chr(0xBB).chr(0xBF));
    fputcsv($out, [
        'Название', 'Кабинет', 'Статус', 'Инв. номер', 'IP', 'MAC',
        'Производитель', 'Модель', 'Серийный номер', 'Год производства',
        'Ввод в эксплуатацию', 'Гарантия до',
        'Тип подключения', 'Поддержка сети',
...
<div class="content-wrapper">
    <div class="content-container">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="mb-0" style="text-align:left">Принтеры</h1>
            <div class="d-flex gap-2">
                <a href="?<?= http_build_query(array_filter(array_merge($filters, ['export'=>'csv']))) ?>" class="btn btn-outline-success">⬇ CSV</a>
                <a href="add.php" class="btn btn-outline-success">➕ Добавить принтер</a>
            </div>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-3" style="font-size:13px;color:#6b7499">
            <span id="count-label">Всего: <?= count($items) ?> принтеров</span>
        ...
        <?php if (empty($items)): ?>
            <p class="p-center">Принтер не найдены.</p>
        <?php else: ?>
            <div class="table-responsive">
                <table id="main-table">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr onclick="location.href='edit.php?id=<?= $item['id'] ?>&from=printers'" style="cursor:pointer">
