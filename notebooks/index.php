...
$sql  = "SELECT d.*, r.name AS room_name,
         nh.cpu, nh.ram_gb, nh.hdd_gb, nh.gpu, nh.os,
         nh.manufacturer, nh.model, nh.serial_number, nh.year_manufactured,
         nh.commissioned_at, nh.warranty_until, nh.floor,
         (SELECT COUNT(*) FROM notebook_issues ci WHERE ci.device_id=d.id AND ci.resolved_at IS NULL) AS open_issues
         FROM devices d
         LEFT JOIN rooms r ON r.id = d.room_id
         LEFT JOIN notebook_hardware nh ON nh.device_id = d.id";
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="notebooks_' . date('Y-m-d') . '.csv"');
...
<div class="content-wrapper">
    <div class="content-container">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="mb-0" style="text-align:left">Ноутбуки</h1>
            <div class="d-flex gap-2">
                <a href="?<?= http_build_query(array_filter(array_merge($filters, ["export"=>"csv"]))) ?>" class="btn btn-outline-success">⬇ CSV</a>
                <a href="add.php" class="btn btn-outline-success">➕ Добавить ноутбук</a>
            </div>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-3" style="font-size:13px;color:#6b7499">
            <span id="count-label">Всего: <?= count($items) ?> ноутбуков</span>
        ...
        <?php if (empty($items)): ?>
            <p class="p-center">Ноутбуки не найдены.</p>
        <?php else: ?>
            <div class="table-responsive">
                <table id="main-table">
                    ...
                    <tbody>
                        ...
                        ?>
                            <tr onclick="location.href='edit.php?id=<?= $item['id'] ?>&from=notebooks'" style="cursor:pointer">
