...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    elseif (!$room_id)  $error = 'Необходимо выбрать кабинет.';
        ...
        $pdo->prepare("INSERT INTO devices (room_id,name,type,ip,mac,inventory_number,status,comment,icon) VALUES (?,?,?,?,?,?,?,?,?)")
            ->execute([$room_id, $name, 'Сервер', $ip, $mac, $inventory, $status, $comment, $icon]);
        ...
        try {
            $pdo->prepare("INSERT INTO server_history (device_id,action,note) VALUES (?,?,?)")
                ->execute([$new_id, 'created', "$name — Сервер, кабинет #$room_id"]);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="mb-0">Добавить сервер</h1>
...
<div class="form-section">
    ...
    <div class="row">
        <div class="col-md-5" style="margin-bottom:14px">
            ...
            <input type="text" name="hw_cpu" class="form-control" placeholder="Intel Xeon E5-2680 v4" value="<?= htmlspecialchars($_POST['hw_cpu'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">ОЗУ (ГБ)</label>
            <input type="text" name="hw_ram" class="form-control" placeholder="64" value="<?= htmlspecialchars($_POST['hw_ram'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Диск</label>
            <input type="text" name="hw_hdd" class="form-control" placeholder="2×1TB RAID1" value="<?= htmlspecialchars($_POST['hw_hdd'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">ОС / Гипервизор</label>
            <input type="text" name="hw_os" class="form-control" placeholder="Proxmox VE / Windows Server 2022" value="<?= htmlspecialchars($_POST['hw_os'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:0">
            ...
            <select name="hw_form_factor" class="form-select">
                ...
                <?php foreach (['1U','2U','3U','4U','Tower','Blade','Mini-Tower'] as $ff): ?>
                    <option <?= (($_POST['hw_form_factor']??'')===$ff)?'selected':'' ?>><?= $ff ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
