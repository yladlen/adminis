...
$from     = $_GET['from'] ?? $_POST['from'] ?? 'computers';
$back_url = '/adminis/servers/';
...
$stmt = $pdo->prepare("SELECT d.*, r.name AS room_name FROM devices d JOIN rooms r ON d.room_id=r.id WHERE d.id=? AND d.type='Сервер'");
$stmt->execute([$device_id]);
$device = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$device) die("Сервер не найден.");

// Железо
$hw = $pdo->prepare("SELECT * FROM server_hardware WHERE device_id=?");
...
// Открытые проблемы
$issuesByComp = [];
$stmt2 = $pdo->prepare("SELECT component FROM server_issues WHERE device_id=? AND resolved_at IS NULL");
$stmt2->execute([$device_id]);
foreach ($stmt2->fetchAll() as $row) if ($row['component']) $issuesByComp[$row['component']] = true;

$openIssues = $pdo->prepare("SELECT * FROM server_issues WHERE device_id=? AND resolved_at IS NULL ORDER BY reported_at DESC");
$openIssues->execute([$device_id]);
$openIssues = $openIssues->fetchAll(PDO::FETCH_ASSOC);

// История
$history = $pdo->prepare("SELECT * FROM server_history WHERE device_id=? ORDER BY created_at DESC LIMIT 50");
...
$componentLabels = [
    'fan'=>'Вентилятор','hdd'=>'Жёсткий диск','psu'=>'Блок питания','ram'=>'ОЗУ',
    'motherboard'=>'Материнская плата','maintenance'=>'Необходимо ТО',
    'os_errors'=>'Ошибки ОС','sw_errors'=>'Ошибки ПО','other'=>'Прочее',
];
$historyActions = [
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_issue'])) {
        ...
        if ($desc !== '') {
            $pdo->prepare("INSERT INTO server_issues (device_id,component,description) VALUES (?,?,?)")
                ->execute([$device_id, $comp ?: null, $desc]);
            $pdo->prepare("INSERT INTO server_history (device_id,action,field_name,new_value) VALUES (?,?,?,?)")
    ...
    if (isset($_POST['resolve_issue'])) {
        ...
        $pdo->prepare("UPDATE server_issues SET resolved_at=NOW(),resolution=? WHERE id=? AND device_id=?")
            ->execute([$resolution ?: null, $issue_id, $device_id]);
        $iss = $pdo->prepare("SELECT component,description FROM server_issues WHERE id=?");
        $iss->execute([$issue_id]);
        $issRow = $iss->fetch(PDO::FETCH_ASSOC);
        $pdo->prepare("INSERT INTO server_history (device_id,action,field_name,old_value,new_value) VALUES (?,?,?,?,?)")
    ...
    if (isset($_POST['update'])) {
        ...
        $writeoff_date = ($writeoff && empty($device['recommended_for_writeoff']))
            ? date('Y-m-d')  // ставим дату только при первом включении
        ...
        if ($name === '') { $error = 'Название обязательно.'; }
            ...
            $hwFields = [
                'hw_cpu'          => ['key'=>'cpu',              'label'=>'Процессор'],
                'hw_ram'          => ['key'=>'ram_gb',           'label'=>'ОЗУ'],
                'hw_hdd'          => ['key'=>'hdd_gb',           'label'=>'Диск'],
                'hw_os'           => ['key'=>'os',               'label'=>'ОС/Гипервизор'],
                'hw_form_factor'  => ['key'=>'form_factor',      'label'=>'Форм-фактор'],
            ...
            if (!empty($changes)) {
                ...
                $pdo->prepare("INSERT INTO server_history (device_id,action,field_name,old_value,new_value,note) VALUES (?,?,?,?,?,?)")
            ...
            $pdo->prepare("INSERT INTO server_hardware
                (device_id,cpu,ram_gb,hdd_gb,os,form_factor,manufacturer,model,serial_number,year_manufactured,commissioned_at,warranty_until,floor)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)
                ON DUPLICATE KEY UPDATE
                cpu=VALUES(cpu),ram_gb=VALUES(ram_gb),hdd_gb=VALUES(hdd_gb),os=VALUES(os),
                form_factor=VALUES(form_factor),manufacturer=VALUES(manufacturer),model=VALUES(model),
                serial_number=VALUES(serial_number),year_manufactured=VALUES(year_manufactured),
                commissioned_at=VALUES(commissioned_at),warranty_until=VALUES(warranty_until),floor=VALUES(floor)")
    ...
    if (isset($_POST['duplicate'])) {
        $pdo->prepare("INSERT INTO devices (room_id,name,type,ip,mac,inventory_number,status,comment,icon) VALUES (?,?,?,?,?,?,?,?,?)")
            ->execute([$device['room_id'],$device['name'].' (копия)','Сервер',$device['ip'],$device['mac'],$device['inventory_number'],$device['status'],$device['comment'],$device['icon']]);
        ...
        if ($hw) {
            ...
            $pdo->prepare("INSERT INTO server_hardware (device_id,$keys) VALUES (?,$vals)")
                ->execute(array_merge([$new_id], array_values($hw)));
        }
        $pdo->prepare("INSERT INTO server_history (device_id,action,note) VALUES (?,?,?)")
...
<div class="d-flex justify-content-between align-items-center mb-4" style="gap:16px;flex-wrap:wrap">
    <!-- Левая часть: название + бейдж проблем -->
    ...
    <div style="display:flex;align-items:center;gap:6px;flex-wrap:nowrap">
        ...
        <button type="submit" name="delete" class="btn btn-outline-danger" onclick="return confirm('Удалить компьютер?')">🗑️ Удалить</button>
        <button type="submit" name="duplicate" class="btn btn-outline-success" onclick="return confirm('Дублировать компьютер?')">📋 Дублировать</button>

        <div style="width:1px;height:28px;background:#e5e7ef;margin:0 8px;flex-shrink:0"></div>

        <!-- Группа: сохранить + отмена -->
...
<div class="form-section">
    ...
    <div class="row">
        ...
        <div class="col-md-5" style="margin-bottom:14px">
            ...
            <input type="text" name="hw_manufacturer" class="form-control" placeholder="Lenovo, HP..." value="<?= htmlspecialchars($hw['manufacturer'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Модель</label>
            <input type="text" name="hw_model" class="form-control" placeholder="ThinkCentre M720" value="<?= htmlspecialchars($hw['model'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
...
<div class="form-section">
    ...
    <div class="row">
        <div class="col-md-5" style="margin-bottom:14px">
            ...
            <input type="text" name="hw_cpu" class="form-control" placeholder="Intel Xeon E5-2680 v4" value="<?= htmlspecialchars($hw['cpu'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">ОЗУ (ГБ)</label>
            <input type="text" name="hw_ram" class="form-control" placeholder="64" value="<?= htmlspecialchars($hw['ram_gb'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Диск</label>
            <input type="text" name="hw_hdd" class="form-control" placeholder="2×1TB RAID1" value="<?= htmlspecialchars($hw['hdd_gb'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">ОС / Гипервизор</label>
            <input type="text" name="hw_os" class="form-control" placeholder="Proxmox VE / Windows Server 2022" value="<?= htmlspecialchars($hw['os'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:0">
            <label class="form-label">Форм-фактор</label>
            <select name="hw_form_factor" class="form-select">
                <option value="">— не указан —</option>
                <?php foreach (['1U','2U','3U','4U','Tower','Blade','Mini-Tower'] as $ff): ?>
                    <option <?= ($hw['form_factor']??'')===$ff?'selected':'' ?>><?= $ff ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
...
<div class="side-panel">
    ...
    <div class="panel-box">
        ...
        <div>
            ...
            <?php if (!empty($hw['cpu'])): ?>
                <div class="passport-row">
                    <span class="passport-label">ЦП</span>
                    <span class="passport-value" style="font-size:12px"><?= htmlspecialchars($hw['cpu']) ?></span>
                </div>
            <?php endif; ?>
            <?php if (!empty($hw['ram_gb'])): ?>
                <div class="passport-row">
                    <span class="passport-label">ОЗУ</span>
                    <span class="passport-value" style="font-size:12px"><?= htmlspecialchars($hw['ram_gb']) ?> ГБ</span>
                </div>
            <?php endif; ?>
            <?php if (!empty($hw['hdd_gb'])): ?>
                <div class="passport-row">
                    <span class="passport-label">Диск</span>
                    <span class="passport-value" style="font-size:12px"><?= htmlspecialchars($hw['hdd_gb']) ?></span>
                </div>
            <?php endif; ?>
            <?php if (!empty($hw['os'])): ?>
                <div class="passport-row">
                    <span class="passport-label">ОС</span>
                    <span class="passport-value" style="font-size:12px"><?= htmlspecialchars($hw['os']) ?></span>
                </div>
            <?php endif; ?>
            <?php if (!empty($hw['form_factor'])): ?>
                <div class="passport-row">
                    <span class="passport-label">Форм-фактор</span>
                    <span class="passport-value" style="font-size:12px"><?= htmlspecialchars($hw['form_factor']) ?></span>
                </div>
            <?php endif; ?>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('/adminis/assets/icons/?json=1').catch(()=>null); // не используем
    // Загружаем иконки напрямую из /assets/icons/
    fetch('/adminis/load_icons.php?dir=1').then(r=>r.json()).catch(()=>[]).then(files => {
