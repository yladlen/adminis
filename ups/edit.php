...
$stmt = $pdo->prepare("SELECT d.*, r.name AS room_name FROM devices d JOIN rooms r ON d.room_id=r.id WHERE d.id=? AND d.type='ИБП'");
$stmt->execute([$device_id]);
$device = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$device) die("ИБП не найден.");

// Железо
$hw = $pdo->prepare("SELECT * FROM ups_hardware WHERE device_id=?");
$hw->execute([$device_id]);
$hw = $hw->fetch(PDO::FETCH_ASSOC) ?: [];

// Открытые проблемы
$issuesByComp = [];
$stmt2 = $pdo->prepare("SELECT component FROM ups_issues WHERE device_id=? AND resolved_at IS NULL");
$stmt2->execute([$device_id]);
foreach ($stmt2->fetchAll() as $row) if ($row['component']) $issuesByComp[$row['component']] = true;

$openIssues = $pdo->prepare("SELECT * FROM ups_issues WHERE device_id=? AND resolved_at IS NULL ORDER BY reported_at DESC");
$openIssues->execute([$device_id]);
$openIssues = $openIssues->fetchAll(PDO::FETCH_ASSOC);

// История
$history = $pdo->prepare("SELECT * FROM ups_history WHERE device_id=? ORDER BY created_at DESC LIMIT 50");
...
$componentLabels = [
    'battery'     => 'Батарея',
    'overload'    => 'Перегрузка',
    'no_signal'   => 'Нет связи/сигнала',
    'fan'         => 'Вентилятор',
    'outlet'      => 'Розетка/выход',
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_issue'])) {
        ...
        if ($desc !== '') {
            $pdo->prepare("INSERT INTO ups_issues (device_id,component,description) VALUES (?,?,?)")
                ->execute([$device_id, $comp ?: null, $desc]);
            $pdo->prepare("INSERT INTO ups_history (device_id,action,field_name,new_value) VALUES (?,?,?,?)")
    ...
    if (isset($_POST['resolve_issue'])) {
        ...
        $pdo->prepare("UPDATE ups_issues SET resolved_at=NOW(),resolution=? WHERE id=? AND device_id=?")
            ->execute([$resolution ?: null, $issue_id, $device_id]);
        $iss = $pdo->prepare("SELECT component,description FROM ups_issues WHERE id=?");
        $iss->execute([$issue_id]);
        $issRow = $iss->fetch(PDO::FETCH_ASSOC);
        $pdo->prepare("INSERT INTO ups_history (device_id,action,field_name,old_value,new_value) VALUES (?,?,?,?,?)")
    ...
    if (isset($_POST['update'])) {
        ...
        if ($name === '') { $error = 'Название обязательно.'; }
            ...
            $hwFields = [
                'hw_power_va'     => ['key'=>'power_va',         'label'=>'Мощность (ВА)'],
                'hw_battery_type' => ['key'=>'battery_type',     'label'=>'Тип батареи'],
                'hw_battery_date' => ['key'=>'battery_replaced', 'label'=>'Замена батареи'],
            ...
            if (!empty($changes)) {
                ...
                $pdo->prepare("INSERT INTO ups_history (device_id,action,field_name,old_value,new_value,note) VALUES (?,?,?,?,?,?)")
            ...
            // Характеристики ИБП
            $hwData = [
                trim($_POST['hw_power_va']     ?? ''),
                trim($_POST['hw_battery_type'] ?? ''),
                trim($_POST['hw_battery_date'] ?? '') ?: null,
            ...
            $pdo->prepare("INSERT INTO ups_hardware
                (device_id,power_va,battery_type,battery_replaced,manufacturer,model,serial_number,year_manufactured,commissioned_at,warranty_until,floor)
                VALUES (?,?,?,?,?,?,?,?,?,?,?)
                ON DUPLICATE KEY UPDATE
                power_va=VALUES(power_va),battery_type=VALUES(battery_type),battery_replaced=VALUES(battery_replaced),
    ...
    if (isset($_POST['duplicate'])) {
        $pdo->prepare("INSERT INTO devices (room_id,name,type,ip,mac,inventory_number,status,comment,icon) VALUES (?,?,?,?,?,?,?,?,?)")
            ->execute([$device['room_id'],$device['name'].' (копия)','ИБП',$device['ip'],$device['mac'],$device['inventory_number'],$device['status'],$device['comment'],$device['icon']]);
        $new_id = $pdo->lastInsertId();
        if ($hw) {
            unset($hw['id'], $hw['device_id']);
            $keys = implode(',', array_keys($hw));
            $vals = implode(',', array_fill(0, count($hw), '?'));
            $pdo->prepare("INSERT INTO ups_hardware (device_id,$keys) VALUES (?,$vals)")
                ->execute(array_merge([$new_id], array_values($hw)));
        }
        $pdo->prepare("INSERT INTO ups_history (device_id,action,note) VALUES (?,?,?)")
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div style="display:flex;align-items:center;gap:6px;flex-wrap:nowrap">
        ...
        <button type="submit" name="delete" class="btn btn-outline-danger" onclick="return confirm('Удалить ИБП?')">🗑️ Удалить</button>
        <button type="submit" name="duplicate" class="btn btn-outline-success" onclick="return confirm('Дублировать ИБП?')">📋 Дублировать</button>
...
<div class="form-section">
    ...
    <div class="row">
        ...
        <div class="col-md-5" style="margin-bottom:14px">
            ...
            <input type="text" name="hw_manufacturer" class="form-control" placeholder="APC, Eaton, CyberPower..." value="<?= htmlspecialchars($hw['manufacturer'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Модель</label>
            <input type="text" name="hw_model" class="form-control" placeholder="Smart-UPS 1500..." value="<?= htmlspecialchars($hw['model'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
        ...
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Мощность (ВА)</label>
            <input type="text" name="hw_power_va" class="form-control" placeholder="1500" value="<?= htmlspecialchars($hw['power_va'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Тип батареи</label>
            <input type="text" name="hw_battery_type" class="form-control" placeholder="AGM 12V 9Ah..." value="<?= htmlspecialchars($hw['battery_type'] ?? '') ?>">
        </div>
        <!-- Строка 4 -->
        <div class="col-md-5" style="margin-bottom:0">
            <label class="form-label">Дата замены батареи</label>
            <input type="date" name="hw_battery_date" class="form-control" value="<?= htmlspecialchars($hw['battery_replaced'] ?? '') ?>">
        </div>
    ...
    <div class="panel-box">
        ...
        <div>
            ...
            $passport = [
                ...
                'Мощность'      => ($hw['power_va'] ?? '') ? $hw['power_va'].' ВА' : '',
                'Батарея'       => $hw['battery_type'] ?? '',
            ];
            ...
            <?php if (!empty($hw['battery_replaced'])): ?>
                <div class="passport-row">
                    <span class="passport-label">Замена батареи</span>
                    <span class="passport-value"><?= date('d.m.Y', strtotime($hw['battery_replaced'])) ?></span>
                </div>
            <?php endif; ?>
