...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    elseif (!$room_id)  $error = 'Необходимо выбрать кабинет.';
        ...
        $pdo->prepare("INSERT INTO devices (room_id,name,type,ip,mac,inventory_number,status,comment,icon) VALUES (?,?,?,?,?,?,?,?,?)")
            ->execute([$room_id, $name, 'ИБП', $ip, $mac, $inventory, $status, $comment, $icon]);
        ...
        try {
            $pdo->prepare("INSERT INTO ups_history (device_id,action,note) VALUES (?,?,?)")
                ->execute([$new_id, 'created', "$name — ИБП, кабинет #$room_id"]);
...
<div class="form-section">
    ...
    <div class="row">
        ...
        <div class="col-md-5" style="margin-bottom:14px">
            ...
            <input type="text" name="hw_manufacturer" class="form-control" placeholder="APC, Eaton, CyberPower..." value="<?= htmlspecialchars($_POST['hw_manufacturer'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Модель</label>
            <input type="text" name="hw_model" class="form-control" placeholder="Smart-UPS 1500..." value="<?= htmlspecialchars($_POST['hw_model'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
        ...
        <div class="col-md-5" style="margin-bottom:14px">
            ...
            <input type="text" name="hw_power_va" class="form-control" placeholder="1500" value="<?= htmlspecialchars($_POST['hw_power_va'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Тип батареи</label>
            <input type="text" name="hw_battery_type" class="form-control" placeholder="AGM 12V 9Ah..." value="<?= htmlspecialchars($_POST['hw_battery_type'] ?? '') ?>">
        </div>
        <!-- Строка 4 -->
        <div class="col-md-5" style="margin-bottom:0">
            <label class="form-label">Дата замены батареи</label>
            <input type="date" name="hw_battery_date" class="form-control" value="<?= htmlspecialchars($_POST['hw_battery_date'] ?? '') ?>">
        </div>
