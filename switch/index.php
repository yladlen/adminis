...
$sql  = "SELECT d.*, r.name AS room_name,
         sh.device_type, sh.ports, sh.port_speed, sh.managed,
         sh.manufacturer, sh.model, sh.serial_number, sh.year_manufactured,
         sh.commissioned_at, sh.warranty_until, sh.floor,
         (SELECT COUNT(*) FROM switch_links sl WHERE sl.connected_to_device_id = d.id) AS connected_count,
         (SELECT COUNT(*) FROM switch_issues si WHERE si.device_id=d.id AND si.resolved_at IS NULL) AS open_issues
         FROM devices d
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    header('Content-Disposition: attachment; filename="switch_' . date('Y-m-d') . '.csv"');
    $out = fopen('php://output', 'w');
    fprintf($out, chr(0xEF).chr(0xBB).chr(0xBF));
    fputcsv($out, [
        'Название', 'Кабинет', 'Этаж', 'Статус', 'Инв. номер', 'IP', 'MAC',
        'Тип устройства', 'Производитель', 'Модель', 'Серийный номер', 'Год производства',
        'Ввод в эксплуатацию', 'Гарантия до',
        'Кол-во портов', 'Скорость портов', 'Управляемый', 'Подключено устройств',
        'Комментарий', 'К списанию', 'Дата рекомендации к списанию', 'Открытых проблем',
    ], ';');
...
<head>
    ...
    <style>
        ...
        .conn-badge { display:inline-flex; align-items:center; gap:3px; font-size:11px; font-weight:600; color:#2563eb; background:#eff6ff; border:1px solid #bfdbfe; border-radius:20px; padding:2px 8px; }
        .managed-badge { display:inline-block; font-size:10px; font-weight:700; padding:1px 6px; border-radius:4px; }
        .managed-yes { background:#dcfce7; color:#16a34a; }
        .managed-no  { background:#f1f5f9; color:#94a3b8; }
        ...
        #main-table th:nth-child(7) { width:90px; }
        #main-table th:nth-child(8) { width:80px; }
        #main-table th:nth-child(9) { width:80px; }
...
<div class="content-wrapper">
    <div class="content-container">
        ...
        <div class="filters-container">
            <div class="d-flex gap-2" style="flex-wrap:wrap;align-items:center">
                <input type="text" id="search-input" name="search" class="form-control" style="flex:2;min-width:200px"
                       ...
                       placeholder="Поиск по названию, IP, инв. номеру...">
        ...
        <?php if (empty($items)): ?>
            <p class="p-center">Устройства не найдены.</p>
        <?php else: ?>
                ...
                <table id="main-table">
                    <thead>
                        <tr>
                            ...
                            <th class="sortable" data-col="5">Тип<i class="sort-icon"></i></th>
                            <th class="sortable text-center" data-col="6">Портов<i class="sort-icon"></i></th>
                            <th class="text-center">Упр.</th>
                            <th class="text-center">Подкл.</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        ?>
                            <tr onclick="location.href='edit.php?id=<?= $item['id'] ?>'" style="cursor:pointer">
                                ...
                                <td class="text-center"><?= htmlspecialchars($item['room_name'] ?? '—') ?></td>
                                <td class="text-center"><?= htmlspecialchars($item['floor'] ?? '—') ?></td>
                                <td style="font-family:monospace;font-size:12px;color:#4f6ef7"><?= htmlspecialchars($item['ip'] ?? '') ?></td>
                                <td><?= htmlspecialchars($item['device_type'] ?? '—') ?></td>
                                <td class="text-center"><?= $item['ports'] ? htmlspecialchars($item['ports']) : '—' ?></td>
                                <td class="text-center">
                                    <span class="managed-badge <?= $item['managed'] ? 'managed-yes' : 'managed-no' ?>">
                                        <?= $item['managed'] ? 'Да' : 'Нет' ?>
                                    </span>
                                </td>
                                <td class="text-center">
                                    <?php if ($item['connected_count'] > 0): ?>
                                        <span class="conn-badge">🔌 <?= $item['connected_count'] ?></span>
                                    <?php else: ?>
