...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    elseif (!$room_id) $error = 'Необходимо выбрать кабинет.';
        ...
        $pdo->prepare("INSERT INTO devices (room_id,name,type,ip,mac,inventory_number,status,comment,icon) VALUES (?,?,?,?,?,?,?,?,?)")
            ->execute([$room_id, $name, 'Принтер', $ip, $mac, $inventory, $status, $comment, $icon]);
        ...
        $pdo->prepare("INSERT INTO printer_passport
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?)")
            ->execute(array_merge([$new_id], $ppData));

        try {
            $pdo->prepare("INSERT INTO printer_history (device_id,action,note) VALUES (?,?,?)")
                ->execute([$new_id, 'created', "$name — Принтер, кабинет #$room_id"]);
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1 class="mb-0">Добавить принтер</h1>
...
<div class="form-section">
    ...
    <div class="row">
        ...
        <div class="col-md-5" style="margin-bottom:14px">
            ...
            <input type="text" name="pp_floor" class="form-control" placeholder="1, 2..." value="<?= htmlspecialchars($_POST['pp_floor'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
        ...
        <div class="col-md-5" style="margin-bottom:14px">
            ...
            <input type="text" name="pp_manufacturer" class="form-control" placeholder="Canon, HP, Kyocera..." value="<?= htmlspecialchars($_POST['pp_manufacturer'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Модель</label>
            <input type="text" name="pp_model" class="form-control" placeholder="i-SENSYS MF735Cx..." value="<?= htmlspecialchars($_POST['pp_model'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Серийный номер</label>
            <input type="text" name="pp_serial" class="form-control" value="<?= htmlspecialchars($_POST['pp_serial'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:14px">
            <label class="form-label">Год производства</label>
            <input type="number" name="pp_year" class="form-control" placeholder="2020" min="1990" max="2099" value="<?= htmlspecialchars($_POST['pp_year'] ?? '') ?>">
        </div>

        <!-- Строка 3 -->
        <div class="col-md-5" style="margin-bottom:0">
            <label class="form-label">Ввод в эксплуатацию</label>
            <input type="date" name="pp_commissioned" class="form-control" value="<?= htmlspecialchars($_POST['pp_commissioned'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:0">
            <label class="form-label">Гарантия до</label>
            <input type="date" name="pp_warranty" class="form-control" value="<?= htmlspecialchars($_POST['pp_warranty'] ?? '') ?>">
        </div>
        <div class="col-md-5" style="margin-bottom:0">
            ...
            <select name="pp_connection" class="form-select">
                ...
                <?php foreach (['USB','LAN','Wi-Fi','LAN + USB','Wi-Fi + USB'] as $ct): ?>
                    <option <?= (($_POST['pp_connection']??'')===$ct)?'selected':'' ?>><?= $ct ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="col-md-5" style="margin-bottom:14px;display:flex;align-items:flex-end">
            <label style="display:inline-flex;align-items:center;gap:8px;cursor:pointer;padding:7px 12px;border:1px solid #dee2e6;border-radius:6px;background:#fff;width:100%">
                <input type="checkbox" name="pp_network" id="pp_network" style="width:16px;height:16px;accent-color:#4f6ef7;cursor:pointer" <?= !empty($_POST['pp_network'])?'checked':'' ?>>
